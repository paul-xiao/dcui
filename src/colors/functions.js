...
module.exports = {
  ...
  injectThemes: function (addBase, config, themes) {
    ...
    // add custom themes
    if (Array.isArray(config("dcv.themes"))) {
      config("dcv.themes").forEach((item, index) => {
    ...
    let themeOrder = [];
    if (Array.isArray(config("dcv.themes"))) {
      config("dcv.themes").forEach((theme, index) => {
    ...
    } else if (config("dcv.themes") != false) {
      themeOrder = [
        "light",
        "dark",
        "cupcake",
    ...
    themeOrder.forEach((themeName, index) => {
      if (index === 0) {
        ...
        // auto dark
        if (config("dcv.darkTheme")) {
          if (
            themeOrder[0] != config("dcv.darkTheme") &&
            themeOrder.includes(config("dcv.darkTheme"))
          ) {
            addBase({
              ["@media (prefers-color-scheme: dark)"]: {
                [":root"]:
                  includedThemesObj[
                  `[data-theme=${config("dcv.darkTheme")}]`
                  ],
              },
            });
          }
